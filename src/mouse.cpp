...
glm::vec2 slop::Mouse::getMousePos() {
  ...
  int wx, wy;
  unsigned int mask;
  XQueryPointer(x11->display, x11->root, &root, &child, &mx, &my, &wx, &wy,
                &mask);
...
slop::Mouse::~Mouse() { XUngrabPointer(x11->display, CurrentTime); }
    ...
    if (a <= rect.z && a >= 0) {
      if (b <= rect.w && b >= 0) {
        ...
        case 1:
          ...
          XQueryTree(x11->display, selectedWindow, &root, &parent, &children,
                     &nchildren);
          if (!children || nchildren <= 0) {
            return selectedWindow;
          }
