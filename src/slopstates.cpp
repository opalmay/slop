...
void slop::SlopMemory::draw(glm::mat4 &matrix) { state->draw(*this, matrix); }
...
void slop::SlopStart::onEnter(SlopMemory &memory) { setStartPos = false; }
void slop::SlopStart::update(SlopMemory &memory, double dt) {
  ...
  if (setStartPos &&
      glm::distance(startPoint, mouse->getMousePos()) >= memory.tolerance) {
    memory.setState((SlopState *)new SlopStartDrag(startPoint));
  }
  if (mouse->hoverWindow != None) {
    glm::vec4 rect =
        getWindowGeometry(mouse->hoverWindow, memory.nodecorations);
    memory.rectangle->setPoints(
        glm::vec2((float)rect.x, (float)rect.y),
        glm::vec2((float)rect.x + rect.z, (float)rect.y + rect.w));
  }
...
slop::SlopStartDrag::SlopStartDrag(glm::vec2 point) { startPoint = point; }
...
void slop::SlopStartDrag::update(SlopMemory &memory, double dt) {
  ...
  //             break;
  // }
  // Compensate for edges of screen, depending on the mouse position in relation
  // to the start point.
  ...
  if (keyboard) {
    if (keyboard->getKey(XK_space)) {
      memory.setState(
          (SlopState *)new SlopStartMove(startPoint, mouse->getMousePos()));
      return;
    }
    int arrows[2];
...
void slop::SlopEndDrag::onEnter(SlopMemory &memory) { memory.running = false; }
